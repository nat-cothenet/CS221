...
####################
	...
	for inputs1 in train_data_obtained: # train_data_obtained is the list of sentences which contains all info like username, reivews, ratings, etc
		...
		int_val_rating = get_y(rating)
		if int_val_rating!= None : #  getIntValue() returns -1 for examples where rating extraction was unsuccessful
			# train_set.append([(only_review _x,int_val_rating)])
			train_set_x.append(only_review)
			train_set_y.append(int_val_rating)
			# train_set[1].append([int_val_rating])
			#Acount[int_val_rating] +=1
			# int_value_of_rating= getIntValue(rating)
				# count= count+1
	# print ("count_zeros= " + str(count[0]) + " count_ones= " + str(count[1]))
	# print ("count_two= " + str(count[2]) + " count_three= " + str(count[3]))
	# print ("count_four= " + str(count[4]) + " count_five= " + str(count[5]))
...
## feature vecture is a dictonary that contains all the words in the dataset as keys and values= 0. All the words are converted to lower case
	...
	# word_index is a dictionary that has index of each word as its value
	word_index = {}

### generate corpus creates feature_vec dictionary 
	#feature_vec= geenerate_corpus(train_set_x)
	word_index = generate_index(train_set_x)
	...
	for y in range(0,len(train_set_x)):
		...
		dict_of_sent= return_sentence_list(train_set_x[y], word_index)
		...
		features.append(dict_of_sent)


	#print features

	# reviews is of size n*s where n is the number of reviews, s is our vocabulary size
	# ratings is of size n*1

	#read in the data, this is jsut peudo-code
	...
	logit.fit(features, train_set_y)
	logit.score(features, train_set_y)

	#correctly classified
	accuracy = np.mean(logit1.predict(features)==train_set_y)
...
def return_sentence_list(string_passed, vocab_dict):
	wordList = re.sub("[^\w]", " ",  string_passed).split()
	sentence_vec = array.array('i', [0,]*len(vocab_list))
	for words in wordList:
